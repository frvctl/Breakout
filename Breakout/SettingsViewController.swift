...
class SettingsViewController: UITableViewController {
    ...
    @IBOutlet weak var brickLabel: UILabel! {
        didSet {
            brickLabel.text = "\(defaults.integerForKey("bricksPerRow"))"
        }
    }
    
    @IBOutlet weak var brickStepper: UIStepper! {
        didSet {
            brickStepper.value = Double(defaults.integerForKey("bricksPerRow"))
        }
    }

    @IBOutlet weak var bouncinessSlider: UISlider! {
        didSet {
            bouncinessSlider.value = defaults.floatForKey("ballBounciness")
        }
    }
    
    @IBOutlet weak var specialSwitch: UISwitch! {
        didSet {
            specialSwitch.on = defaults.boolForKey("specialBricks")
        }
    }
    
    @IBOutlet weak var ballSegmentedControl: UISegmentedControl! {
        didSet {
            ballSegmentedControl.selectedSegmentIndex = defaults.integerForKey("numberOfBalls") - 1
        }
    }
